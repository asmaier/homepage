...
def extract_topics(text):
    ...
    topics = eval(result["text"])
    # remove Kategorie: string sometimes generated by the LLM
    fixed_topics = [topic.replace("Kategorie:", "") for topic in topics]
    # remove strings consisting only of whitespace
    cleaned_topics = [topic for topic in fixed_topics if topic.strip()]
...
if __name__ == "__main__":
    ...
    file_paths = []
    # find all html files
    for path in Path(PATH_POSTS).rglob('*.html'):
        file_paths.append(path)
    # find all markdown files except the branch bundle _index.md files
